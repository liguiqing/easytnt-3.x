...
CREATE TABLE `t_ps_Subject` (
  ...
  `score` DOUBLE(6,2)  COMMENT '满分', 
  `kgScore` DOUBLE(6,2)  COMMENT '客观题分',
  `zgScore` DOUBLE(6,2)  COMMENT '主观题分',       
...
CREATE TABLE `t_ps_AnswerSheetItem` (
  ...
  `catagory1` TINYINT(1)  COMMENT '题目一级类型：1－客观题(KG)；2－主观题(ZG)',
...
CREATE TABLE `t_ps_ExamineeSheet` (
  ...
  `score` DOUBLE(6,2)  COMMENT '整卷得分', 
  `kgScore` DOUBLE(6,2)  COMMENT '客观题得分',
  `zgScore` DOUBLE(6,2)  COMMENT '主观题得分',    
  PRIMARY KEY (`id`), 
...
CREATE TABLE `t_ps_MarkTeam` (
  ...
  `seq` tinyint(1) DEFAULT NULL COMMENT '同层次组序号',
  PRIMARY KEY (`id`), 
...
CREATE TABLE `t_ps_ExamNumberMark` (
  ...
  `timesRequired` TINYINT(2)  COMMENT '必须完成的评阅次数',
  `timesFinished` TINYINT(2) COMMENT '已完成的评阅次数' ,
...
CREATE TABLE `t_ps_ItemStatistics` (
  ...
  `statsValue` DOUBLE(6,2)  COMMENT '统计指标值' , 
