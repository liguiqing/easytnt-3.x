...
CREATE TABLE `ts_teacher` (
  ...
  `periodStarts` DATE (16)  COMMENT '入职时间，为空时表示一直在职',
  `periodEnds` DATE (16)  COMMENT '离职时间,为空时表示一直在职',
...
DROP TABLE IF EXISTS `ts_headmaster`;
CREATE TABLE `ts_headmaster` (
  `id` BIGINT(20)  NOT NULL AUTO_INCREMENT ,
  `schoolId` varchar(36) NOT NULL COMMENT '所属学校唯一标识',
  `personId` varchar(36) NOT NULL COMMENT '人员唯一标识，系统中所有人员是唯一的，如果有两个值相同，表示同一个人员',
  `identity` varchar(32) NOT NULL COMMENT '校长身份证件号',
  `identityType` SMALLINT (2) DEFAULT 1  COMMENT '校长身份证件号类型：身份证-1, 学籍号-2, 学号-3,教育云标识-4,QQ-5,微信-6,港澳台证件号-7, 考号-8,其他-99',
  `identityTypeName` varchar (8) DEFAULT `身份证`  COMMENT '校长身份证件名，与identityType保持一致',
  `gender` varchar(4) DEFAULT `无` COMMENT '校长性别',
  `periodStarts` DATE (16)  COMMENT '入职时间，为空时表示一直在职',
  `periodEnds` DATE (16)  COMMENT '离职时间,为空时表示一直在职',
  PRIMARY KEY (`id`),
  KEY `x_ts_headmaster_schoolId` (`schoolId`),
  KEY `x_ts_headmaster_personId` (`personId`)
)ENGINE=INNODB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COMMENT='校长信息表';
...
CREATE TABLE `ts_clazz_master` (
  ...
  `periodStarts` DATE (16)  COMMENT '入职时间，为空时表示一直在职',
  `periodEnds` DATE (16)  COMMENT '离职时间,为空时表示一直在职',
  `clazzId` varchar(36) NOT NULL COMMENT '负责班主唯一标识',
  PRIMARY KEY (`id`),
